...
export default async function handler(req,res){
  ...
  try{
    ...
    const i=String(req.query.interval||'1h');
    const limit=Math.min(Math.max(Number(req.query.limit)||120,30),200);
    if(!s.endsWith('USDT')||!allowed.has(i))return res.status(400).json({ok:false,error:'invalid args'});
    const now=Date.now();
    let startTime;
    if(i==='1M'){
      const d=new Date(now);
      d.setUTCMonth(d.getUTCMonth()-(limit+3));
      startTime=d.getTime();
    }else{
      startTime=Math.max(0,now-(limit+3)*ms[i]);
    }
    const apiLimit=Math.min(limit+3,200);
    const q=new URLSearchParams({symbol:s,interval:i,startTime:String(startTime),endTime:String(now),limit:String(apiLimit),type:'LAST_PRICE'});
    ...
    if(!r.ok||Number(j.code)!==0)throw new Error('kline error');
    const rows=(j.data||[]).slice().sort((a,b)=>Number(a.time)-Number(b.time)).slice(-limit);
    const candles=rows.map((x,idx)=>{
      const quoteVolume=n(x.quoteVol??x.quoteVolume??x.turnover??x.amount);
      const baseVolume=n(x.baseVol??x.baseVolume??x.volume);
      return {
        time:Number(x.time),
    ...
    if(candles.length){
      ...
      if(i==='1M'){
        ...
        last.isClosed=now>=Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,1);
      }else{
        last.isClosed=now>=last.time+ms[i];
      }
    }
